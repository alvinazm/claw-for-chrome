...
function drawClickIndicator(ctx, x, y, scaleFactor = 1) {
  ...
  // Outer glow (scaled)
  ctx.beginPath();
  ctx.arc(x, y, scaleFactor * 15, 0, Math.PI * 2);
  ctx.fillStyle = "rgba(207, 107, 60, 0.3)";
  ctx.fill();

  // Inner circle (scaled)
  ctx.beginPath();
  ctx.arc(x, y, scaleFactor * 11, 0, Math.PI * 2);
...
function drawDragPath(ctx, startX, startY, endX, endY, scaleFactor = 1) {
  ...
  const arrowLength = scaleFactor * 15;
  ctx.beginPath();
  ...
  ctx.beginPath();
  ctx.arc(startX, startY, scaleFactor * 6, 0, Math.PI * 2);
  ...
  ctx.beginPath();
  ctx.arc(endX, endY, scaleFactor * 6, 0, Math.PI * 2);
  ctx.fillStyle = "#ffffff";
  ctx.fill();
  ctx.strokeStyle = "#dc2626";
  ctx.lineWidth = scaleFactor * 2;
...
function drawActionLabel(ctx, text, x, y, scaleFactor = 1) {
  ...
  const textHeight = scaleFactor * 20;
  const padding = scaleFactor * 8;

  // Adjust position if too close to edges (scaled offsets)
  let labelX = x + scaleFactor * 20;
  let labelY = y - scaleFactor * 10;
  if (labelX + textWidth + padding * 2 > ctx.canvas.width) {
    labelX = x - textWidth - padding * 2 - scaleFactor * 20;
  }
  if (labelY < 0) {
    labelY = y + scaleFactor * 20;
  }
  ...
  ctx.shadowBlur = scaleFactor * 4;
  ctx.shadowOffsetX = 0;
  ctx.shadowOffsetY = scaleFactor * 2;
...
function drawWatermark(ctx, scaleFactor = 1) {
  ctx.save();
  const padding = scaleFactor * 8;
  const logoSize = scaleFactor * 32;
  const x = ctx.canvas.width - padding - logoSize;
  const y = ctx.canvas.height - padding - logoSize - scaleFactor * 4;
...
function applyActionIndicators(canvas, action, options, scaleFactor = 1) {
  ...
  if (options.showClickIndicators && action.coordinate && (action.type.includes("click") || action.type === "scroll")) {
  ...
  // Draw drag path
  if (options.showDragPaths && action.type === "left_click_drag" && action.start_coordinate && action.coordinate) {
  ...
  if (options.showActionLabels && action.description && !action.coordinate && (action.type === "type" || action.type === "key" || action.type === "wait")) {
    // Place label in top-left area (scaled position)
    drawActionLabel(ctx, action.description, scaleFactor * 20, scaleFactor * 20, scaleFactor);
  }
}
...
async function generateGif(frames, options = {}) {
  ...
  const enhancedCanvases = images.map((img, index) => {
    ...
    if (frame.viewportWidth && canvas.width) {
      ...
      console.log(`[Offscreen] Frame ${index + 1}: Applying scale factor ${scaleFactor} (canvas: ${canvas.width}x${canvas.height}, viewport: ${frame.viewportWidth}x${frame.viewportHeight})`);
    } else {
      console.log(`[Offscreen] Frame ${index + 1}: No viewport metadata, using scale factor 1.0 (backwards compatibility)`);
    }
    if (frame.action) {
    ...
    console.log(`[Offscreen] Frame ${index + 1}/${images.length} enhanced (progress: ${Math.round(progress * 100)}%)`);
    return canvas;
  });
